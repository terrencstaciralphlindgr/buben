...
export default class threeCommas {
  ...
  async makeRequest(method, path, params) {
    ...
    try {
      let response = await fetch(
        `${this._url}${path}${stringify(params)}`,
        {
          method: method,
          timeout: 30000,
          agent: '',
  ...
  async getBotStatsByDate(params) {
    return await this.makeRequest(
      'GET',
      `/public/api/ver1/bots/stats_by_date?`,
      params,
    );
  }
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  async accountsTypes(params) {
    return await this.makeRequest(
      'GET',
      `/public/api/ver1/accounts/types_to_connect?`,
      params,
    );
  }
...
// console.log(JSON.stringify(await tc.showDeals(1889046959)));
// console.log(await tc.botDelete(9923729));
// console.log(await tc.getAcc({ account_id: 32083983 }));

// const deals = await tc.getDeals({
//   from: '2022-10-20',
//   bot_id: 9923729,
// });
// console.log(deals);
// console.log(
//   deals.reduce((sum, current) => sum + +current.usd_final_profit, 0),
// );
